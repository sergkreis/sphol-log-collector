...
After a fault/process exit, a new explicit Start may replay only the last persisted observed envelope, using its original run ID, source identity, generation and offset. Bytes written beyond that ceiling while closed are skipped. A line completed outside that envelope is not backfilled. Normal Stop/close ends the capture checkpoint and the next Start uses EOF. Existing queued records survive either path. No background service or automatic capture is introduced.

Recovery requires the source still to exist with the matching filesystem identity and prefix fingerprint. Deleted or replaced source bytes cannot be recovered from cursor metadata. Writes after the last observed ceiling are outside the recovery promise. This is not a full private Gamelogs archive. File identity/truncation and UTF-8 behavior require native Windows acceptance in addition to Linux tests.
...
2. Refresh the cross-version fixture from its existing v0.3.3 pin to the verified current stable v0.3.4 artifact/digest. This candidate does not claim the existing older fixture proves a v0.3.4 upgrade. Keep downloads/EXE testing in isolated native CI with outbound collector egress blocked.
