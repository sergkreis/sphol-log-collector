...
class PairingUX:
    def init_pairing_ux(self):
        ...
        self.browser_button = ttk.Button(self.pairing_panel, text='Открыть подтверждение в браузере', command=self.open_pairing_browser)
        self.browser_button.pack(anchor='w', pady=(4, 0))
        self.link_button = ttk.Button(self.pairing_panel, text='Скопировать ссылку', command=self.copy_pairing_link)
